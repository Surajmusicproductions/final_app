...
(function premiumPurchaseWiring(){
  ...
  // --- Buy handlers (calls Supabase Edge Functions) ---
  async function startCheckout(planKey){
    ...
    const { data: orderRes, error: orderErr } =
      await supabase.functions.invoke('create_order', { body: { plan: planKey } });

    if (orderErr || !orderRes?.order) {
      return alert('Failed to create order: ' + (orderErr?.message || orderRes?.error || 'unknown'));
    }
    ...
    const rzp = new window.Razorpay({
      ...
      handler: async function (payload){
        // On success, call Supabase verify_payment function
        const { data: verifyRes, error: verifyErr } =
          await supabase.functions.invoke('verify_payment', {
            body: {
